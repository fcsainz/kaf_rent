...
2. **Zona Piscina/Jardín**: tabla visual con las reservas de ese tipo (columnas a definir en una iteración posterior).
3. **Zona Habitación Interior**: tabla visual con las reservas de ese tipo (columnas a definir en una iteración posterior).

Esta separación es puramente de presentación: a nivel de datos, ambos tipos siguen viviendo en la misma tabla `Reservas`, diferenciados por la columna Tipo; aquí solo se decide cómo se organiza visualmente la pantalla principal, no el modelo de datos.
