...
7. **Importe del alquiler**: importe que paga el huésped por el espacio (sin los servicios extra), introducido manualmente (no existe tarifa fija por espacio; cada reserva la fija el anfitrión). Obligatorio, ≥ 0; se almacena en `Importe_Alquiler`. A partir de él, de la comisión del canal y de los servicios seleccionados, el formulario muestra un **resumen económico orientativo en tiempo real**:
